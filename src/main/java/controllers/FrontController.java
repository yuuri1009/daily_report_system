...
public class FrontController extends HttpServlet {
     ...
     * （例：パラメータがaction＝Employeeの場合、action.EmployeeActionのオブジェクト）
     * @param request リクエスト
    ...
    @SuppressWarnings({ "rawtypes", "unchecked" }) //コンパイラ警告を抑制
     private ActionBase getAction(HttpServletRequest request, HttpServletResponse response) {
         ...
         try {
             ...
             //該当するActionオブジェクトを作成（例：リクエストからパラメータaction.EmployeeActionの場合、actions.EmployeeActionオブジェクト)
             type = Class.forName(String.format("action.%sAction", actionString));
             
             //ActionBaseのオブジェクトにキャスト（例：actions.EmployeeActonオブジェクト actions.ActionBaseオブジェクト）
